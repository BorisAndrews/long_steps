...
def energy_form(u, f, gamma):
    """Constrained optimization energy: max(f - u, 0) + (1/(2γ)) |∇u|^2"""
    constraint_violation = conditional(gt(f - u, 0), f - u, 0)
    return constraint_violation + 0.5 / Constant(gamma) * dot(grad(u), grad(u))


def gradient_direction(u, f, gamma):
    """
    Compute H1-seminorm gradient direction g of E at current u with g=0 on ∂Ω:
    ...
    # d/du[max(f - u, 0)] = -H(f - u) where H is Heaviside
    heaviside = conditional(gt(f - u, 0), 1.0, 0.0)
    L = -heaviside * v * dx + (1.0 / gamma_c) * dot(grad(u), grad(v)) * dx
    a = inner(grad(g), grad(v)) * dx
...
def gradient_descent_constrained(nx=128, degree=1, gamma=2**9,
    ...
    # Descent loop
    Lip = Constant(2 + 1/gamma)  # Rough Lipschitz estimate
    for k in range(1, max_iters + 1):
        # Compute gradient direction g
        g = gradient_direction(u, f, gamma)
