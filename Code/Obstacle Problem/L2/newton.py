...
Compute stationary states for the constrained optimization problem by minimizing:
    ...
    E(u) = (1/2) ∫_Ω max(f - u, 0)^2 dx + (1/(2γ)) ∫_Ω |∇u|^2 dx
...
def solve_constrained(nx=2**7, degree=2, gamma=2**16,
    ...
    # Energy density: (1/2) max(f - u, 0)^2 + (1/(2γ)) |∇u|^2
    # Use conditional to handle max(f - u, 0)
    constraint_violation = conditional(gt(f - u, 0), f - u, 0)
    psi = 0.5 * constraint_violation**2 + 0.5 / gamma_c * dot(grad(u), grad(u))
    E_form = psi * dx
    ...
    E_final = assemble((0.5 * constraint_viol_final**2 + 0.5 / gamma_c * dot(grad(u), grad(u))) * dx)
...
def parse_args():
    ...
    p.add_argument("--gamma", type=float, default=2**16, help="Gradient penalty coefficient (1/interface width^2)")
